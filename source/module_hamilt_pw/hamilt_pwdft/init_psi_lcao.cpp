...
void Wavefunc_in_pw::cal_ovlp_flzjlq(
	std::vector<std::string> &fn, 
	ModuleBase::realArray &table_local)
{
	ModuleBase::TITLE("init_wfc","cal_ovlp_flzjlq");

	if( fn.size() != static_cast<size_t>(GlobalC::ucell.ntype) )
	{
		ModuleBase::WARNING_QUIT("init_wfc::cal_ovlp_flzjlq","maybe NUMERICAL_ORBITAL is not read in, please check.");
	}
	...
	for(int it=0; it<GlobalC::ucell.ntype; it++)
	{
		...
		if(!in)
		{
			...
			ModuleBase::WARNING_QUIT("init_wfc::cal_ovlp_flzjlq","Can not find file.");
		}
		else
		{
			std::stringstream ss;
...
double Wavefunc_in_pw::cosine_interpolation(const double &energy_x,
                               const double &ecut,
                               const double &beta)
{
    double w = 0.0;
...
void Wavefunc_in_pw::init_psi_lcao(const int& ik,
                                 const ModulePW::PW_Basis_K* wfc_basis,
                                 const Structure_Factor& sf,
                                 psi::Psi<std::complex<double>>& psi,
                                 const ModuleBase::realArray& table_local)
{
	ModuleBase::TITLE("init_wfc","cal_ovlp_flzpw");
	assert(ik>=0);
