                        ...
                        if(GlobalC::ucell.atoms[it].ncpp.has_so)
                        {
                            ...
                            {//atomic_wfc_so
                                ...
                                for(int m=-l-1;m<l+1;m++)
                                {
                                   ...
                                   if (fabs(fact[0])>1e-8||fabs(fact[1])>1e-8) // valid LS coupling effect
                                   {
                                      for(int is=0;is<2;is++) // loop over spin
                                      {
                                          if(fabs(fact[is])>1e-8) // specifically if the present spin is valid
                                          {
