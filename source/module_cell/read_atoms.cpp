		...
		for (int it = 0;it < ntype; it++)
		{
			...
			else if(GlobalV::BASIS_TYPE == "pw")
			{
				if ((GlobalV::psi_initializer)&&(GlobalV::init_wfc.substr(0, 3) == "nao"))
				{
					std::string orbital_file = GlobalV::global_orbital_dir + orbital_fn[it];
					this->read_orb_file(it, orbital_file, ofs_running, &(atoms[it]));
					this->read_nao_flz(it, orbital_file, ofs_running, &(atoms[it]));
				}
				else
				{
					this->atoms[it].nw = 0;
...
void UnitCell::read_nao_flz(int it, std::string &orb_file, std::ofstream &ofs_running, Atom* atom)
{
	std::ifstream ifs(orb_file.c_str(), std::ios::in);  // pengfei 2014-10-13
	// mohan add return 2021-04-26
	if(!ifs)
	{
		GlobalV::ofs_warning << " File name : " << orb_file << std::endl;
		ModuleBase::WARNING_QUIT("psi_initializer::cal_ovlp_flzjlq","Can not find file.");
	}
	else
	{
		std::stringstream ss;
	...
	for(int l = 0; l < this->atoms[it].nwl+1; l++)
	{
		for(int izeta = 0; izeta < this->atoms[it].l_nchi[l]; izeta++)
		{
			...
			{
				GlobalV::ofs_warning<<" File name : "<<orb_file<<std::endl;
				ModuleBase::WARNING_QUIT("UnitCell::read_nao_flz","Can not find file.");
			}
			int n_rgrid = 0;
			double dr = 0.0;
			char word[80];
			...
			while( !find )
			{
				if(in.eof())
				{
					GlobalV::ofs_warning<<"\n Can't find l="<<l<< " n=" <<izeta<<" orbital."<<std::endl;
				}
				in >> title_1 >> title_2 >> title_3;
				assert( title_1 == "Type" );
				in >> it_read >> l_read >> n_read;
				if( l == l_read && izeta == n_read )
				{
				...
				{
					double discarded_contents;
					for(int ir=0; ir<meshr_read; ir++)
					{
						 in >> discarded_contents;
					}
				}
			}
			in.close();
